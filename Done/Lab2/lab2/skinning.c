...
void DeformCylinder()
{
	...
	for (row = 0; row < kMaxRow; row++)
	{
		for (corner = 0; corner < kMaxCorners; corner++)
		{
			...
			vec3 temp1 = VectorSub(g_vertsOrg[row][corner], g_bones[0].pos);
			temp1 = MultVec3(g_bones[0].rot, temp1);
			temp1 = VectorAdd(g_vertsRes[row][corner], g_bones[0].pos);
			temp1 = ScalarMult(temp1, 1.0 - weight[row]);
			//temp1 = ScalarMult(temp1, 1.0 - g_boneWeights[row][corner]);

			vec3 temp2 = VectorSub(g_vertsOrg[row][corner], g_bones[1].pos);
			temp2 = MultVec3(g_bones[1].rot, temp2);
			temp2 = VectorAdd(temp2, g_bones[1].pos);
			temp2 = ScalarMult(temp2, weight[row]);
			//temp2 = ScalarMult(temp1, g_boneWeights[row][corner]);
...
void reshape(GLsizei w, GLsizei h)
{
    ...
    //glUniformMatrix4fv(glGetUniformLocation(g_shader, "projMatrix"), 1, GL_TRUE, projectionMatrix.m);
