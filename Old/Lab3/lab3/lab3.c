...
// gcc lab3.c ../common/*.c ../common/*.m -framework OpenGL -framework Cocoa -o lab3 -I../common
...
void updateWorld()
{
    ...
    GLfloat elasticity = 1.0;

	for (i = 0; i < kNumBalls; i++)
        for (j = i+1; j < kNumBalls; j++)
        {
            //Kolla om kollision
            GLfloat dist = Norm(VectorSub(ball[j].X, ball[i].X));

            vec3 impact = VectorSub(ball[j].X, ball[i].X);

            GLfloat vRelTemp = DotProduct(impact, VectorSub(ball[j].P, ball[i].P));

            if(dist < (kBallSize*2) && vRelTemp < 0.0)
            {
                vec3 vi = VectorAdd(ball[i].P,CrossProduct(ball[i].omega, impact));
                vec3 vj = VectorAdd(ball[j].P,CrossProduct(ball[j].omega, impact));

                GLfloat vRel = DotProduct(VectorSub(vi, vj), impact);

                //GLfloat massI = 1/ball[i].mass;
                //GLfloat massJ = 1/ball[j].mass;
                GLfloat massI = ball[i].mass;
                GLfloat massJ = ball[j].mass;

                //vRel = 2.0;

                GLfloat J = -1*(elasticity + 1)*vRel;
                J = J*(massI+massJ);
            

                ball[i].P = VectorAdd(ball[i].P, ScalarMult(impact, J));
                //ball[i].P = ScalarMult(ball[i].P, -1.0);
                ball[j].P = VectorAdd(ball[j].P, ScalarMult(impact, J));
                //ball[j].P = ScalarMult(ball[j].P, -0.5);
                ball[j].P = ScalarMult(ball[j].P, -1.0);
	...
	{
		// simplified
        vec3 up = SetVector(0.0, 1.0, 0.0);
        ball[i].omega = CrossProduct(up, ball[i].v);
...
void init()
{
    ...
    ball[1].X = SetVector(0.0, 0, 0.2);
    ball[2].X = SetVector(0.0, 0, 0.4);
    ball[3].X = SetVector(0.0, 0, 0.6);
	/*ball[1].X = SetVector(0.0, 0, 0.5);
	ball[2].X = SetVector(0.0, 0, 1.0);
	ball[3].X = SetVector(0.0, 0, 1.5);*/
